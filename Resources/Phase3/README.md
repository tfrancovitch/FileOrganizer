...
| `fixture_lab\Fixtures\F01…F15` | Nine of the fifteen P3.R11 synthetic fixtures — the five that test Build 1–2 scope (F01 keeper + protected root + hard link; F02 multiple intentional keepers; F03 folder priority + explicit exception; F09 supersession / undo / reapply; F15 projection rebuild), the two for Build 3 (F08 deferred versus evidence-blocked; F10 revalidation after an observation changed) and the two for Build 4 (F04 a frozen query-result batch that does not acquire a later match; F05 a policy made before a file existed covering it with no decision). Each is a SQLite file with synthetic evidence, authoritative Phase 3 rows and an `expected_projection` oracle. | `p3_fixture_check.py` |
...
| `fixture_lab\MANIFEST.json` | The research's manifest of the full fifteen-fixture lab, with SHA-256 per file. The thirty files shipped here match it exactly; the other entries belong to fixtures for Builds 5–7 and are not shipped. | provenance |
...
(`obs:OBS1`) becomes a `[file_path_id, observation_id]` pair, its
`p3_review_event` rows load as the product's review events (a `time`
deferral's date sits in `return_condition` there), and its `p3_bulk_batch`
/ `p3_bulk_member` rows load as the product's batches (the lab predates the
per-member disposition; every member there is one the batch decided). The
product's own schema is `Scripts\Database\migrations\009_phase3_decisions.sql`,
`010_phase3_review_events.sql` and `011_phase3_bulk_batches.sql`.
